...
def build_infographic_module(data_dict, is_forecast=False):
    if is_forecast:
        ...
        <div class="infographic-module">
            ...
            <div class="info-pivot">
                <h4 style="color:#F5F0E6;">EXECUTIVE PIVOT & PROTOCOL RATIONALE</h4>
...
def build_and_render_pdf(session_data, eng_data, timeline_data, pdf_path):
    ...
    <div class="content">
        ...
        <div class="grid-2col" style="align-items: center;">
            ...
            <div>
                <table class="data-table"><tr><th>Planet / Graha</th><th>Sign</th><th>Degree</th><th>Nakshatra</th><th>Condition</th></tr>{eph_rows}</table>
            </div>
        </div>
        ...
        <div class="synopsis" style="margin-top:15px;"><strong>Curator's Note:</strong> The rationale for these specific physical protocols is detailed in the 'Executive Pivot' sections of this report.</div>
    </div>
    """
    ...
    @page { 
        size: letter; 
        margin: 2.54cm; 
        margin-top: 4cm; /* Increased to prevent header overlap */
    ...
    .cover-page { height: 100vh; margin-top: -4cm; }
    
    /* Fixed overlapping header issue */
    .fixed-header { position: fixed; top: -3.2cm; left: 0; right: 0; text-align: center; }
    ...
    /* ENHANCED INFOGRAPHIC CSS */
    .infographic-module { display: flex; flex-direction: column; gap: 12px; margin-bottom: 20px;}
    .info-content { background: #ffffff; padding: 15px; border-left: 3px solid #101827; border-top: 1px solid #f0f0f0; border-right: 1px solid #f0f0f0; border-bottom: 1px solid #f0f0f0;}
    .info-pivot { background: #0b111a; color: #F5F0E6; padding: 15px; border-left: 3px solid #B68A3A; }
    ...
    .synopsis { font-size: 9pt; color: #58708C; background: #fdfbf7; padding: 10px; border-left: 3px solid #B68A3A; margin-top: 15px; margin-bottom: 20px; font-style: italic; }
