...
def generate_master_pdf(report_text, pdf_path, birth_details_str, name_str, d1_svg, d9_svg):
    try:
        ...
        def get_scaled_drawing(svg_path, target_width=280):
        ...
        for line in report_text.split('\n'):
            ...
            if safe_line.startswith("# PART 1"):
                ...
                try:
                    d1_drawing = get_scaled_drawing(d1_svg)
                    d9_drawing = get_scaled_drawing(d9_svg)
                    if d1_drawing and d9_drawing:
                ...
                story.append(Spacer(1, 12))
            elif safe_line.startswith("# PART 3"):
                story.append(PageBreak())
                story.append(Paragraph("COMPATIBILITY & SYNESTRY ANALYSIS", h1_style))
                story.append(Spacer(1, 12))
            elif safe_line.startswith("*Disclaimer:"):
                story.append(Spacer(1, 6))
                story.append(Paragraph(safe_line, disclaimer_style))
                story.append(Spacer(1, 10))
...
def calculate_chart_logic(asc_sign, planets_full, birth_dt):
    ...
    if age < 18: life_stage = f"CHILD (Age {age}): STRICTLY focus on House 4 (Home), House 5 (Education), House 9 (Mentorship). NO career/marriage predictions."
    elif 18 <= age <= 25: life_stage = f"YOUNG ADULT (Age {age}): Focus on House 9 (Higher Ed), House 10 (Early Career), House 1 (Identity). No marriage timelines yet."
    elif 26 <= age <= 40: life_stage = f"ESTABLISHMENT (Age {age}): Deep dive into House 10 (Career), House 7 (Marriage), House 2 (Wealth), House 6 (Disease/Debt)."
    elif 41 <= age <= 60: life_stage = f"CONSOLIDATION (Age {age}): Focus on House 11 (Gains), House 2 (Savings), House 8 (Longevity), House 10 (Authority). Address mid-life crises."
    else: life_stage = f"ELDER (Age {age}): STRICTLY focus on House 9 (Spirituality), House 12 (Moksha), House 8 (Pension), House 4 (Peace). NO aggressive career growth."
    ...
    for p_name, p_data in planets_full.items():
        for h_num, h_data in houses.items():
            if h_data["sign"] == p_data["sign"]:
                h_data["occupants"].append(p_name); break
    for p_name, p_data in planets_full.items():
    ...
    if asc_lord_dignity.startswith("Debilitated"): psych_triggers.append(f"The Lagna Lord ({asc_lord}) is Debilitated. This severely weakens the user's core vitality, self-esteem, and ability to execute plans independently.")
    if asc_lord_house in [6, 8, 12]: psych_triggers.append(f"The Lagna Lord is placed in a Dusthana (House {asc_lord_house}). This indicates the user's life path is fraught with overcoming obstacles, health issues, or hidden enemies.")

    moon_house = get_house_of_planet(houses, "Moon (Chandra)")
    mercury_house = get_house_of_planet(houses, "Mercury (Budh)")
    if moon_house in [6, 8, 12]: psych_triggers.append("Moon in Dusthana indicates severe emotional volatility and suppressed anxiety.")
    if planets_full["Moon (Chandra)"]["dignity"].startswith("Debilitated"): psych_triggers.append("Debilitated Moon indicates chronic depressive loops and emotional hypersensitivity.")
    if mercury_house in [8, 12]: psych_triggers.append("Mercury in 8th/12th creates nervous system burnout, paranoia, and over-analysis.")
    ...
    if mars_house in [1, 4, 7, 8, 12]: threats.append(f"Vector: MANGAL DOSHA | Trigger: Mars in House {mars_house} | Meaning: Creates aggressive friction in marriage.")
    ...
    if has_kemadruma: special_yogas.append("Kemadruma Yoga (Moon with no adjacent planets): Indicates severe psychological isolation.")

    if any(m in h8_occ for m in MALEFICS): threats.append("Vector: HOSPITALIZATION | Trigger: Malefics in 8th House | Meaning: Risk of sudden trauma/surgery.")
    if houses[10]["ruler_placed_in"] in [8, 12]: threats.append("Vector: CAREER TERMINATION | Trigger: 10th Lord in 8th/12th House | Meaning: Sudden job loss.")
    ...
    if "Venus (Shukra)" in h12_occ and "Rahu" in h12_occ: threats.append("Vector: EXTRA-MARITAL | Trigger: Venus+Rahu in 12th | Meaning: Risk of clandestine affairs.")
    ...
    if "Jupiter (Guru)" in h8_occ or houses[8]["ruler_placed_in"] in [1, 5, 9, 11]: opportunities.append("Vector: SUDDEN WEALTH | Trigger: 8th Lord well-placed | Meaning: Sudden unearned wealth/inheritance.")
...
def calculate_synastry(p1_data, p2_data):
    """Calculates basic Ashtakoot (Nadi, Bhakoot, Gan) compatibility."""
    ...
    # 1. Nadi Dosha (Health/Genetics) - 8 points
    nadi_dosha = (p1_moon_idx % 3) == (p2_moon_idx % 3)
    nadi_pts = 0 if nadi_dosha else 8
    
    # 2. Bhakoot Dosha (Love/Ego) - 7 points
    sign_diff = abs(p1_moon_sign_idx - p2_moon_sign_idx)
    bhakoot_dosha = sign_diff in [1, 2, 6, 7] # 2/12, 3/11, 6/8, 7/7
    bhakoot_pts = 0 if bhakoot_dosha else 7
    
    # 3. Gan Dosha (Temperament) - 6 points
    gan_map = [0, 0, 1, 0, 1, 2, 0, 1, 2, 0, 1, 2, 0, 1, 2, 0, 1, 2, 0, 1, 2, 0, 1, 2, 0, 1, 2] # 0=Deva, 1=Manushya, 2=Rakshasa
    p1_gan = gan_map[p1_moon_idx]; p2_gan = gan_map[p2_moon_idx]
    gan_pts = 6
    ...
    if nadi_dosha: summary += "- NADI DOSHA DETECTED: Identical Nadi groups. High risk of genetic/health issues in progeny and severe emotional misunderstanding.\n"
    if bhakoot_dosha: summary += "- BHAKOOT DOSHA DETECTED: Moon signs in 2/12, 6/8, or 7/7 axis. Creates severe ego conflicts, financial misunderstandings, and emotional distance.\n"
    if gan_pts == 1: summary += "- GAN DOSHA DETECTED: Temperament mismatch (Deva vs Rakshasa). Creates fundamental behavioral friction.\n"
...
def webhook():
    ...
    try:
        ...
        if "message" in update and "text" in update["message"]:
            ...
            # STATE 1: INITIAL CHART INPUT OR PARTNER INPUT
            if match:
                # Check if we are waiting for partner details
                if chat_id in USER_SESSIONS and USER_SESSIONS[chat_id].get("state") == "awaiting_partner":
                    ...
                    p1_data = session["planet_data"]
                    p2_data = p2_planets
                    
                    synastry_str = calculate_synastry(p1_data, p2_data)
                    session["synastry"] = synastry_str
                    session["partner_logic"] = p2_logic
                    session["partner_name"] = name_str2
                    session["state"] = "ready_to_generate"
                    
                    # Trigger final PDF generation
                    return generate_final_pdf(chat_id, session, groq_key, groq_url)
                
                # Otherwise, it's the primary user chart
            ...
            # STATE 2: AWAITING FOCUS AREA
            elif chat_id in USER_SESSIONS and USER_SESSIONS[chat_id].get("state") == "awaiting_focus":
                session = USER_SESSIONS[chat_id]
                if user_text.lower() == 'skip':
                    session["focus_areas"] = "General comprehensive audit."
                else:
                    session["focus_areas"] = user_text
...
3. **Career & Professional Trajectory**
   - Deep dive into the 10th House. Explain real-world impact on job/business.
4. **Wealth & Financial Assets**
   - Deep dive into the 2nd and 11th Houses. Explain wealth capacity and risks.
5. **Marriage & Relationship Dynamics**
   - Deep dive into the 7th House and Venus.
6. **Health & Legal Risk Vectors**
   - Analyze the 6th, 8th, and 12th axis. Explain health/legal vulnerabilities.
...
"""
    ...
    if res.status_code == 200:
        ...
        pdf_success = generate_master_pdf(final_text, pdf_path, session["birth_str"], session["name"], session["d1_svg"], session["d9_svg"])
